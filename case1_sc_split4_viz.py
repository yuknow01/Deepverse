"""
case1_sc_split4_viz.py
─────────────────────
Case 1 SC-wise split4 (scenes=7000, batch=256) 학습 결과 시각화

사용법:
    python case1_sc_split4_viz.py              # N_SCENES=7000 (default)
    python case1_sc_split4_viz.py --scenes 7000
"""
...
def plot_curves(results, title_suffix, out_prefix):
    ...
    ax2.set_ylim(min(vals) - 2, max(vals) + 1)
...
def main():
    ...
    parser.add_argument("--scenes", type=int, default=7000)
    ...
    for name, path in model_files.items():
        if path.exists():
            ...
            d = results[name]
            print(f"[✓] {name:20s}  best val NMSE = {d['best_nmse']:.4f} dB  (epoch {d['best_epoch']})")
        else:
    ...
    title      = f"{SPLIT_TAG}  |  batch=256  |  scenes={N}"
